...
export const useTopNative = () => {
  const [data, setData] = useState<any[any]>(null);

  async function fetch() {
    let tokens: any[] = [];
    const summary = await getTotalSummary(true);
    ...
    for (const property in summary) {
      ...
      tokens.push({
        ...
        name: (
          <Link to={`/native-token/${property}`} className="token-link">
            {token[0]?.name ? (
              <>
    ...
    const topNative = tokens.map((token, index) => {
      return {
        number: index + 1,
        ...token,
      };
